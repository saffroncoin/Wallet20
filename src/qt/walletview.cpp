    ...
    walletModel(0)
{
    ...
    sendBitCoinsPage = new SendBitCoinsDialog(gui);
...
void WalletView::gotoSendBitCoinsPage(QString addr)
{
    gui->getSendBitCoinsAction()->setChecked(true);
    setCurrentWidget(sendBitCoinsPage);

    if (!addr.isEmpty())
        sendBitCoinsPage->setAddress(addr);
}
