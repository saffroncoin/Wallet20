...
// create SFRAnonymize pools
CSFRAnonymizePool sfrAnonymizePool;
CSFRAnonymizeSigner sfrAnonymizeSigner;
std::vector<CMasterNode> sfrAnonymizeMasterNodes;
std::vector<CMasterNodeVote> sfrAnonymizeMasterNodeVotes;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->ssSend.SetVersion(min(pfrom->nVersion, PROTOCOL_VERSION));
         ...
         if (pfrom->nVersion >= sfrAnonymizePool.MIN_PEER_PROTO_VERSION) {
            if(RequestedMasterNodeList <= 5) {
                bool fIsInitialDownload = IsInitialBlockDownload();
                if(!fIsInitialDownload) {
                    pfrom->PushMessage("sfranonyg");
    ...
    else if (strCommand == "verack")
    {
    ...
    else if (strCommand == "sfranonyg") { //SFR Anonymize Get String
        if (pfrom->nVersion != sfrAnonymizePool.MIN_PEER_PROTO_VERSION) {
            return false;
        }
        int count = sfrAnonymizeMasterNodes.size()-1;
        int i = 0;

        BOOST_FOREACH(CMasterNode mn, sfrAnonymizeMasterNodes) {
            printf("Sending master node entry - %s \n", mn.addr.ToString().c_str());
            mn.Check();
            if(mn.IsEnabled()) {
                pfrom->PushMessage("sfranonye", mn.vin, mn.addr, mn.sig, mn.now, mn.pubkey, mn.pubkey2, count, i);
                i++;
            }
        }
    }
    ...
    else if (strCommand == "sfranonye") { //SFR Anonymize Entry String  
        if (pfrom->nVersion != sfrAnonymizePool.MIN_PEER_PROTO_VERSION) {
            return false;
        }
        bool fIsInitialDownload = IsInitialBlockDownload();
        ...
        if(pubkeyScript.size() != 25) {
            printf("sfranonye - pubkey the wrong size\n");
        ...
        if(!sfrAnonymizeSigner.VerifyMessage(pubkey, vchSig, strMessage, errorMessage)){
            printf("sfranonye - Got bad masternode address signature\n");
            pfrom->Misbehaving(100);
            return false;
        }

        if((fTestNet && addr.GetPort() != 29717) || (!fTestNet && addr.GetPort() != 19717)) return true;
        ...
        BOOST_FOREACH(CMasterNode& mn, sfrAnonymizeMasterNodes) {
            ...
            if(mn.vin == vin) {
                if(!mn.UpdatedWithin(5*60*1000)){
                    ...
                    if(count == -1)
                        RelaySFRAnonymizeElectionEntry(vin, addr, vchSig, sigTime, pubkey, pubkey2, count, current);
                }
        ...
        printf("sfranonye - Got NEW masternode entry %s\n", addr.ToString().c_str());

        CValidationState state;
        CTransaction tx = CTransaction();
        CTxOut vout = CTxOut(999.99*COIN, sfrAnonymizePool.collateralPubKey);
        tx.vin.push_back(vin);
        tx.vout.push_back(vout);
        if(tx.AcceptableInputs(state, true)){
            printf("sfranonye - Accepted masternode entry %i %i\n", count, current);

            if(GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS){
                printf("sfranonye - Input must have least %d confirmations\n", MASTERNODE_MIN_CONFIRMATIONS);
            ...
            mn.UpdateLastSeen();
            sfrAnonymizeMasterNodes.push_back(mn);
            
            if(count == -1)
                RelaySFRAnonymizeElectionEntry(vin, addr, vchSig, sigTime, pubkey, pubkey2, count, current); 

        } else {
            printf("sfranonye - Rejected masternode entry\n");
    ...
    else if (strCommand == "sfranonyp") { //SFR Anonymize Entry Ping 
        if (pfrom->nVersion != sfrAnonymizePool.MIN_PEER_PROTO_VERSION) {
            return false;
        }
        bool fIsInitialDownload = IsInitialBlockDownload();
        ...
        if (sigTime/1000000 > GetAdjustedTime() + 5 * 60) {
            printf("sfranonyp: Signature rejected, too far into the future");
            pfrom->Misbehaving(20);
            return false;
        }

        if (sigTime/1000000 <= pindexPrev->GetBlockTime() - 5 * 60) {
            printf("sfranonyp: Signature rejected, too far into the past");
            pfrom->Misbehaving(20);
            return false;
        }
        ...
        BOOST_FOREACH(CMasterNode& mn, sfrAnonymizeMasterNodes) {
            ...
            if(mn.vin == vin) {
                ...
                std::string errorMessage = "";
                if(!sfrAnonymizeSigner.VerifyMessage(mn.pubkey2, vchSig, strMessage, errorMessage)){
                ...
                if(stop) {
                    if(mn.IsEnabled()){
                        mn.Disable();
                        mn.Check();
                        RelaySFRAnonymizeElectionEntryPing(vin, vchSig, sigTime, stop);
                    }
                    return true;
...
// start masternode payments
    ...
    int64 nFees = 0;
    {
        ...
        if(bMasterNodePayment) {
            ...
            if(blockLast.ReadFromDisk(pindexPrev)){
                BOOST_FOREACH(CMasterNodeVote& mv1, blockLast.vmn){
                    ...
                    if(mv1.GetVotes() >= MASTERNODE_PAYMENTS_MIN_VOTES-1){
                        ...
                        BOOST_FOREACH(CMasterNodeVote& mv2, sfrAnonymizeMasterNodeVotes) {
            ...
            int winningNode = sfrAnonymizePool.GetCurrentMasterNode(1);
            if(winningNode >= 0){
                CMasterNodeVote mv;
                mv.Set(sfrAnonymizeMasterNodes[winningNode].pubkey, pindexPrev->nHeight + 1);
...
void CSFRAnonymizePool::SetCollateralAddress(std::string strAddress){
    CBitcoinAddress address;
    if (!address.SetString(strAddress))
        printf("CSFRAnonymizePool::SetCollateralAddress - Invalid SFRAnonymize collateral address\n");

    collateralPubKey.SetDestination(address.Get());
}

//Get last block hash
bool CSFRAnonymizePool::GetLastValidBlockHash(uint256& hash, int mod)
{
...
void CSFRAnonymizePool::NewBlock()
{
    if(fDebug) printf("CSFRAnonymizePool::NewBlock \n");
    ...
    {    
        ...
        if(pindexBest != NULL) {
            ...
            if(winningNode >= 0){
                CMasterNodeVote mv;
                mv.Set(sfrAnonymizeMasterNodes[winningNode].pubkey, pindexBest->nHeight + 1);
                sfrAnonymizeMasterNodeVotes.push_back(mv);

                if(sfrAnonymizeMasterNodeVotes.size() > MASTERNODE_PAYMENTS_EXPIRATION){
                    sfrAnonymizeMasterNodeVotes.erase(sfrAnonymizeMasterNodeVotes.begin(), sfrAnonymizeMasterNodeVotes.end()-MASTERNODE_PAYMENTS_EXPIRATION);
                }
            }
        }
    }
...
uint256 CMasterNode::CalculateScore(int mod)
{
    ...
    uint256 n1 = 0;
    if(!sfrAnonymizePool.GetLastValidBlockHash(n1, mod)) return 0;
...
int CSFRAnonymizePool::GetCurrentMasterNode(int mod)
{
    int i = 0;
...
bool CSFRAnonymizeSigner::SetKey(std::string strSecret, std::string& errorMessage, CKey& key, CPubKey& pubkey){
...
bool CSFRAnonymizeSigner::SignMessage(std::string strMessage, std::string& errorMessage, vector<unsigned char>& vchSig, CKey key)
{
...
bool CSFRAnonymizeSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    CHashWriter ss(SER_GETHASH, 0);
    ss << strMessageMagic;
    ss << strMessage;
